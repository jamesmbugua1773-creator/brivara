...
export default function DepositsSection({ userId, jwt }: { userId: string, jwt: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${base}/admin/deposits?userId=${userId}`, {
      ...
      .then(async (r) => {
        if (!r.ok) throw new Error(t('depositsNotFound'));
        const arr = await r.json();
        setDeposits(arr);
        setError(null);
      })
      .catch((e) => setError(e?.message || t('loadFailedDeposits')))
  ...
  if (loading) return <div>{t('loadingDeposits')}</div>;
        ...
        <>
          <div className="mb-2 flex gap-2 items-center">
            <input
              ...
              placeholder={t('depositsSearchPlaceholder')}
              value={search}
