...
export default function AwardsSection({ userId, jwt }: { userId: string, jwt: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${base}/admin/awards/history?userId=${userId}`, {
      ...
      .then(async (r) => {
        if (!r.ok) throw new Error(t('awardsNotFound'));
        const arr = await r.json();
        setAwards(arr);
        setError(null);
      })
      .catch((e) => setError(e?.message || t('loadFailedAwards')))
  ...
  const handleDelete = async (id: string) => {
    if (!jwt) return;
    if (!confirm(t('confirmDeleteAward'))) return;
    try {
          ...
          {showAdd && (
            <div className="mb-2 flex gap-2 items-center">
              <select className="px-2 py-1 rounded bg-slate-800 border border-slate-700" value={newAward.awardName} onChange={e => setNewAward(n => ({ ...n, awardName: e.target.value }))}>
                <option value="">{t('awardOptionPlaceholder')}</option>
              ...
              <input className="px-2 py-1 rounded bg-slate-800 border border-slate-700" type="number" min="1" placeholder={t('amountPlaceholder')} value={newAward.packageAmount} onChange={e => setNewAward(n => ({ ...n, packageAmount: e.target.value }))} />
              ...
              <table className="min-w-full text-xs">
                <thead>
                  <tr className="bg-slate-800">
                    <th className="p-2">{t('colAward')}</th>
                ...
                <tbody>
                  {filtered.map(a => (
                    <tr key={a.id} className="border-b border-slate-700">
                      ...
                      <td className="p-2">
                        <button className="px-2 py-1 rounded bg-red-700 text-white text-xs" onClick={() => handleDelete(a.id)}>{buttons('delete')}</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
