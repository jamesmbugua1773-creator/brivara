...
import { DashboardView } from '@/app/components/views';
...
export default function DashboardPage() {
  ...
  const [userProfile, setUserProfile] = useState<{ role: string; username: string; name: string; email: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [summaryRes, progressRes, rebateSummaryRes, roiHistoryRes, referralRes, profileRes] = await Promise.all([
        ...
        if (profileRes.ok) {
          ...
          console.error('Failed to fetch dashboard profile:', profileRes.status, profileRes.statusText);
  ...
  return <DashboardView data={data} loading={loading} apiError={apiError} referralLink={referralLink} onCopy={() => { navigator.clipboard.writeText(referralLink?.url || ''); setCopied(true); setTimeout(() => setCopied(false), 800); }} copied={copied} userProfile={userProfile} />;
}
